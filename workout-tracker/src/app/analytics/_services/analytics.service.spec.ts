...
describe.only('AnalyticsService', () => {
  ...
  it.only('should convert date strings to Date objects when getting executed workout metrics', async () => {
    const httpMock = TestBed.inject(HttpTestingController);
    const mockResponse = [
      {
    ...
    let results!: ExecutedWorkoutMetrics[];

    service
      .getExecutedWorkoutMetrics('some-id', 50)
