...
export class ExecutedWorkoutService extends ApiBaseService<ExecutedWorkoutDTO> {
   ...
   */
  public getFilteredSubset(startingIndex: number, pageSize: number, workoutNameContains: string | null = null): Observable<ExecutedWorkoutSummaryDtoPaginatedResults> {
    const result: Observable<ExecutedWorkoutSummaryDtoPaginatedResults> =
      this._http.get<ExecutedWorkoutSummaryDtoPaginatedResults>(`${this._apiRoot}?firstRecord=${startingIndex}&pageSize=${pageSize}${workoutNameContains ? '&workoutNameContains=' + workoutNameContains : ''}`)
        .pipe(
