...
- Declare service mocks as `Partial<Mocked<ConcreteService>>` and create them inside `beforeEach` so a fresh instance is produced for every test, eliminating the need for `vi.clearAllMocks()` between tests. Provide them via `useValue` and retrieve the instance via `TestBed.inject(ConcreteService)`, either inline in individual tests or assigned to a spec-level variable inside `beforeEach`. Type each mock method with the `vi.fn<ConcreteService['methodName']>()` generic parameter syntax and import `{ type Mocked }` from `vitest`. See [src/app/core/welcome/welcome.component.spec.ts](../../src/app/core/welcome/welcome.component.spec.ts) for the canonical example.
